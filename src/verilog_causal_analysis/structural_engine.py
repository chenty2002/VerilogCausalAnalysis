"""Deterministic production engine for the structural causal graph contract."""
...
from .structural_contract import StructuralCausalRequest, STRUCTURAL_GRAPH_SCHEMA
...
def _graph_identity(request: StructuralCausalRequest) -> Dict[str, Any]:
    rtl_rows = [
        {
...
def _empty_graph(
    request: StructuralCausalRequest,
...
def _convert_graph(
    request: StructuralCausalRequest,
    structural_nodes: Iterable[Mapping[str, Any]],
...
def _parsed_design_cache_key(
    ...
    return canonical_sha256(
        {
            "schema_version": "parsed_design_cache_key",
            "rtl_files": sorted(
                (
                    {
                        "artifact_id": artifact.artifact_id,
...
def _write_persistent_cache(
    ...
    if cache_path is None:
        return
    entry = {
        "schema_version": "parsed_design_cache_entry",
        "cache_key": cache_key,
...
class PreparedCausalAnalysis:
    ...
    def __init__(
        self,
        request: StructuralCausalRequest | Mapping[str, Any],
    ):
        if not isinstance(request, StructuralCausalRequest):
            request = StructuralCausalRequest.from_dict(
                request, production=True
            )
        if not request.strict:
            raise ValueError("production structural API requires strict=true")
...
def _build_structural_graph(
    ...
    try:
        with PreparedCausalAnalysis(request) as prepared:
...
def build_structural_graph(
    ...
    """Build a strict structural graph without any auto-detection or path guessing."""
    return _build_structural_graph(request)
